...
$(function() {
	if(window.location.hash.length > 0) {
		...
		request("api/polls.jsp?action=grab", {id: POLL}, function(obj) {
			parseData(obj);
		});
	}
	...
	*/
	$("button.remq").live("click", function() {
		if(questionCount > 1) {
			...
			if(id != "-1") {
				request("api/questions.jsp?action=delete", {id: id});
			}
			questionCount--;
		}
	});
	...
	*/
	$("button.saveq").live("click", function() {
		...
		//send the request to the server
		request("api/questions.jsp?action=update", data, function(resp) {
			self.hide();
			if(resp.newid) {
